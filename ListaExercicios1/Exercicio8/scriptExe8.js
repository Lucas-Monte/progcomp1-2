...
for (let i = 1; i <= 6; i++) {
  ...
  olho = prompt(
    `Qual a cor do olho da ${i}° pessoa? (a-azul; p-preto; v-verde; c-castanho)`
  );
  ...
  cabelo = prompt(
    `Qual a cor do cabelo da ${i}° pessoa? (l-loiro; p-preto; r-ruivo; c-castanho)`
  );
...
alert(
  `Teve ${pessoasMaior50EMenos60} ${palavraPessoa} com mais de 50 anos e menos de 60 kg`
);
...
if (inferior150 == 0) {
  ...
  alert(
    `A média das idades das pessoas com menos de 1,50 m é igual a ${media.toFixed(
      2
    )}`
  );
}
...
alert(
  `A quantidade de pessoas ruivas e que não possuem olhos azuis é ${ruivoMenosAzul}`
);
