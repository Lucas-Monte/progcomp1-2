...
for (let i = 0; i < projetos.length; i++) {
  if (projetos[i].duracao > 12 && projetos[i].orcamento > 1000000) {
    console.log(
      `O projeto ${projetos[i].nome} tem duração maior de 12 meses e orçamento maior que R$ 1.000.000,00`
    );
  }
...
console.log(
  `A média de duração dos projetos é de ${soma / projetos.length} meses`
);
console.log(`Foi realizado ao todo ${inovaTech} projeto(s) pela InovaTech`);
console.log(
  `O total gasto por projetos com menos de 6 meses foi de R$ ${somaOrcamento}`
);
