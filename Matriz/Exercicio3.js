...
for (let i = 0; i < 5; i++) {
  ...
  for (let j = 0; j < 5; j++) {
    do {
      matriz[i][j] = Number(
        prompt(`Digite o indice da regiao ${regiao[i]} no dia ${dia[j]}:`)
      );
...
console.log(
  `A regiao com a menor media dos 5 dias foi a região ${menorRegiao} com a media de indices em ${regiao[posicaoMenReg]}`
);
